...
//                 case 'unfollow':// ブロック（そもそもメッセージを送れない...）
//                 case 'join':// LINE公式アカウントがグループトークや複数人トークに参加したことを示すイベントです。参加イベントには応答できます。
//                 case 'case 'leave':// LINE公式アカウント削除 or 退出（そもそもメッセージを送れない...）
...
//                 case 'things':// ユーザーの操作により、デバイスとLINEが連携されたことを示すイベントです。（things.type === link）ユーザーの操作により、デバイスとLINEが連携解除されたことを示すイベントです。（things.type === unlink）自動通信のシナリオが実行されたことを示すイベントです。シナリオセットごとにまとめて実行結果が返されるのではなく、シナリオごとに実行結果が返されます。（things.type === scenarioResult）
                case 'message':
